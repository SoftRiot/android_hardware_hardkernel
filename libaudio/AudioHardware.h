...
public:
    ...
    virtual AudioStreamOut* openOutputStreamWithFlags(
        uint32_t devices, audio_output_flags_t flags=(audio_output_flags_t)0, int *format=0, uint32_t *channels=0,
        uint32_t *sampleRate=0, status_t *status=0);
    ...
    virtual size_t getInputBufferSize(
        uint32_t sampleRate, int format, int channelCount);
            ...
            const AudioMixer *getOutputCloseRouteFromDevice(uint32_t device);
